...
//definición de la estructura de registros
struct articulo
{
	char codart[5];//el código es de 4. Se agrega 1 para el terminador ´\0´
	float pu;
	int meses[12];
	};
//fin de la definición de la estructura de registros
...
int buscar(struct articulo *vart, char *codart )
	...
	for(i=0;i<200;i++)
		if(strcmp(vart[i].codart, codart)==	0)
			return i; //cuando encuentra el código vuelve a la función que llamó

	return -1;//devuelve -1 si no encontró el código en el vector
	}
